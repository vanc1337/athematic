    ...
    <?php foreach ((array) $rows as $row): ?>
      <tr>
        <?php foreach ($row as $cell): ?>
          <?php if(preg_match('/.*today.*/',$cell['class'])){$cal_today_match=1;}else{$cal_today_match=0;}?>
          <td id="<?php print $cell['id']; ?>" class="<?php print $cell['class']; ?>">
            <?php if($cal_today_match==1){/*print '<div id="today_div" class="today-div">';*/} ?> 

            <?php print $cell['data']; ?>
                        <?php if($cal_today_match==1){print '</div>';}?>
